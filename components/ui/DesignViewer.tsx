...
export function DesignViewer({ design }: { design?: Design | null }) {
  ...
  const hasAnnotations = Boolean(design?.annotations?.length);
        ...
        <div className="relative aspect-[1.42] overflow-auto bg-[#edf4f2] p-4">
          ...
          <div className="relative h-full min-h-[360px] w-full origin-center transition" style={{ transform: `scale(${zoom})` }}>
            {design?.design_image_url ? (
              <div className={hasAnnotations ? "absolute left-[16%] top-[8%] h-[84%] w-[68%]" : "absolute inset-0"}>
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={design.design_image_url} alt="Electrical design" className="h-full w-full object-contain" />
              </div>
            ) : (
              <div className="absolute inset-0 grid place-items-center border border-[#cbdeda] bg-[linear-gradient(90deg,#d7e6e3_1px,transparent_1px),linear-gradient(#d7e6e3_1px,transparent_1px)] bg-[size:28px_28px] text-[#1f2a33]">
            ...
            {hasAnnotations && design?.annotations ? <AnnotationOverlay annotations={design.annotations} /> : null}
          </div>
        </div>
      </div>
