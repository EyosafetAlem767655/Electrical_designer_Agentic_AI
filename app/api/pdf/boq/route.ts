...
import { z } from "zod";
import { createBoqPdf } from "@/lib/pdf-utils";
...
export async function POST(request: Request) {
  try {
    ...
    const buffer = await createBoqPdf(project as Project, floor as Floor, design as Design);
    return new Response(buffer, {
      headers: {
        "Content-Type": "application/pdf",
        "Content-Disposition": `attachment; filename="${(floor as Floor).floor_name.replace(/[^a-z0-9]+/gi, "-")}-boq.pdf"`
      }
    });
  } catch (error) {
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : "BOQ PDF export failed" }, { status: 400 });
  }
}
